@extends('layout.admin') {{-- Sesuaikan dengan nama layout admin kamu --}}
...
<div class="p-6 space-y-8">
    ...
    <!-- HEADER & FORM TAMBAH PARTNER -->
    <div class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
        <h2 class="text-xl font-bold text-slate-800 mb-6">Tambah Partner Baru</h2>
        
        <form action="{{ route('admin.partners.store') }}" method="POST" class="flex flex-col md:flex-row gap-4 items-end">
            @csrf
            <div class="flex-1 space-y-2 w-full">
                <label class="text-sm font-bold text-slate-600 ml-1">Nama Partner</label>
                <input type="text" name="name" class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition text-sm" placeholder="Masukkan nama partner" required>
            </div>
            <div class="flex-1 space-y-2 w-full">
                <label class="text-sm font-bold text-slate-600 ml-1">Logo URL</label>
                <input type="text" name="logo_url" class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition text-sm" value="https://placehold.co/200x200" required>
            </div>
            <button type="submit" class="w-full md:w-auto bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl font-bold transition shadow-lg shadow-indigo-100 text-sm">
                Simpan
            </button>
        </form>
    </div>

    <!-- PENCARIAN & FITUR SEARCH -->
    <div class="flex justify-between items-center gap-4">
        <form action="{{ route('admin.partners.index') }}" method="GET" class="flex gap-2 w-full md:w-96">
            ...
            <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-100 text-sm">
                Cari
            </button>
        </form>
    </div>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex justify-center gap-3">
                                ...
                                <a href="{{ route('admin.partners.edit', $partner->id) }}" 
                                   class="w-9 h-9 flex items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white transition shadow-sm">
                                ...
                                <form action="{{ route('admin.partners.destroy', $partner->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus partner ini?')">
                                    ...
                                    @method('DELETE')
                                    <button type="submit" 
                                            class="w-9 h-9 flex items-center justify-center rounded-xl bg-red-50 text-red-600 hover:bg-red-600 hover:text-white transition shadow-sm">
